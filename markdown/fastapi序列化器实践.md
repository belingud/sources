...
def model2schema(
    ...
    *,
    ...
    >>> ModelSerializer = model2schema(Account)
    >>> from collections import namedtuple
    >>> model = namedtuple("Account", ["id", "name"])
    >>> instance = model(id=1, name="demo")
    >>> ModelSerializer.dump(instance)

    >>> from pydantic import validator
    >>> from apps.utils.validate import datetime2timestamp
    >>> class ModelSerializer(model2schema(Account))
    >>>     dt2ts = validator("create_time", allow_reuse=True)(datetime2timestamp)
    :param db_model:
    :param config:
    :param include:
    :param exclude:
